export default {
  0: { name: "None", icon: "" },
  2: {
    name: "Story",
    icon:
      "https://www.bungie.net/common/destiny2_content/icons/DestinyActivityTypeDefinition_397a3349627ce8420e3f8b26cb3d2d2a.png"
  },
  3: {
    name: "Strike",
    icon:
      "https://www.bungie.net/common/destiny2_content/icons/DestinyActivityTypeDefinition_29d738456013ebca7fe89a8155060121.png"
  },
  4: {
    name: "Raid",
    icon:
      "https://www.bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_bfe80e3dafe6686a9dc42df0606bdc9b.png"
  },
  5: {
    name: "AllPvP",
    icon:
      "https://www.bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_fb3e9149c43f7a2e8f8b66cbea7845fe.png"
  },
  6: {
    name: "Patrol",
    icon:
      "https://www.bungie.net/common/destiny2_content/icons/53ccd31da0ab8dedd1fef007463b434c.png"
  },
  7: { name: "AllPvE", icon: "" },
  9: { name: "Reserved9", icon: "" },
  10: {
    name: "Control",
    icon:
      "https://www.bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_922ae83203f6134bae00937d12eab3a2.png"
  },
  11: { name: "Reserved11", icon: "" },
  12: {
    name: "Clash",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_fb3e9149c43f7a2e8f8b66cbea7845fe.png"
  },
  13: { name: "Reserved13", icon: "" },
  15: {
    name: "Crimson - Doubles",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_f9d1438e0536f36e868e691eb0acfc4d.png"
  },
  16: { name: "Nightfall", icon: "" },
  17: { name: "HeroicNightfall", icon: "" },
  18: { name: "AllStrikes", icon: "" },
  19: {
    name: "IronBanner",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_fe57052d7cf971f7502daa75a2ca2437.png"
  },
  20: { name: "Reserved20", icon: "" },
  21: { name: "Reserved21", icon: "" },
  22: { name: "Reserved22", icon: "" },
  24: { name: "Reserved24", icon: "" },
  25: {
    name: "AllMayhem",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_52f64830161b33289d36edc03f6d2ea6.png"
  },
  26: { name: "Reserved26", icon: "" },
  27: { name: "Reserved27", icon: "" },
  28: { name: "Reserved28", icon: "" },
  29: { name: "Reserved29", icon: "" },
  30: { name: "Reserved30", icon: "" },
  31: { name: "Supremacy", icon: "" },
  32: {
    name: "PrivateMatchesAll",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_52f64830161b33289d36edc03f6d2ea6.png"
  },
  37: {
    name: "Survival",
    icon:
      "https://bungie.net/common/destiny2_content/icons/1c17984345f393e728dc8d2b898eca66.png"
  },
  38: {
    name: "Countdown",
    icon:
      "https://bungie.net/common/destiny2_content/icons/1c17984345f393e728dc8d2b898eca66.png"
  },
  39: {
    name: "TrialsOfTheNine",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_f4b12d402bb36270cd911f4e1fa8bb0c.png"
  },
  40: { name: "Social", icon: "" },
  41: {
    name: "Trials - Countdown",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_f4b12d402bb36270cd911f4e1fa8bb0c.png"
  },
  42: {
    name: "Trials - Survival",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_f4b12d402bb36270cd911f4e1fa8bb0c.png"
  },
  43: {
    name: "IronBanner - Control",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_fe57052d7cf971f7502daa75a2ca2437.png"
  },
  44: {
    name: "IronBanner - Clash",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_fe57052d7cf971f7502daa75a2ca2437.png"
  },
  45: {
    name: "IronBanner - Supremacy",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_fe57052d7cf971f7502daa75a2ca2437.png"
  },
  46: { name: "ScoredNightfall", icon: "" },
  47: { name: "ScoredHeroicNightfall", icon: "" },
  48: {
    name: "Rumble",
    icon:
      "https://bungie.net/common/destiny2_content/icons/843744b6902d8672635e3232984cbe29.png"
  },
  49: {
    name: "AllDoubles",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_f9d1438e0536f36e868e691eb0acfc4d.png"
  },
  50: {
    name: "Doubles",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_f9d1438e0536f36e868e691eb0acfc4d.png"
  },
  51: {
    name: "PrivateMatchesClash",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_52f64830161b33289d36edc03f6d2ea6.png"
  },
  52: {
    name: "PrivateMatchesControl",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_52f64830161b33289d36edc03f6d2ea6.png"
  },
  53: {
    name: "PrivateMatchesSupremacy",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_52f64830161b33289d36edc03f6d2ea6.png"
  },
  54: {
    name: "PrivateMatchesCountdown",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_52f64830161b33289d36edc03f6d2ea6.png"
  },
  55: {
    name: "PrivateMatchesSurvival",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_52f64830161b33289d36edc03f6d2ea6.png"
  },
  56: {
    name: "PrivateMatchesMayhem",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_52f64830161b33289d36edc03f6d2ea6.png"
  },
  57: {
    name: "PrivateMatchesRumble",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_52f64830161b33289d36edc03f6d2ea6.png"
  },
  58: { name: "HeroicAdventure", icon: "" },
  59: { name: "Showdown", icon: "" },
  60: { name: "Lockdown", icon: "" },
  61: { name: "Scorched", icon: "" },
  62: { name: "ScorchedTeam", icon: "" },
  63: { name: "Gambit", icon: "" },
  64: { name: "AllPvE - Comp.", icon: "" },
  65: { name: "Breakthrough", icon: "" },
  66: { name: "BlackArmoryRun", icon: "" },
  67: { name: "Salvage", icon: "" },
  68: {
    name: "Iron Banner - Salvage",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_fe57052d7cf971f7502daa75a2ca2437.png"
  },
  69: {
    name: "PvP - Comp.",
    icon:
      "https://bungie.net/common/destiny2_content/icons/1c17984345f393e728dc8d2b898eca66.png"
  },
  70: {
    name: "PvPQuickplay",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_fb3e9149c43f7a2e8f8b66cbea7845fe.png"
  },
  71: {
    name: "Clash - QPlay.",
    icon:
      "https://bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_fb3e9149c43f7a2e8f8b66cbea7845fe.png"
  },
  72: {
    name: "Clash - Comp.",
    icon:
      "https://bungie.net/common/destiny2_content/icons/1c17984345f393e728dc8d2b898eca66.png"
  },
  73: {
    name: "Control - QPlay.",
    icon:
      "https://www.bungie.net/common/destiny2_content/icons/DestinyActivityModeDefinition_922ae83203f6134bae00937d12eab3a2.png"
  },
  74: {
    name: "Control - Comp.",
    icon:
      "https://bungie.net/common/destiny2_content/icons/1c17984345f393e728dc8d2b898eca66.png"
  },
  75: { name: "Gambit - Prime", icon: "" },
  76: { name: "Reckoning", icon: "" }
};
